...
function bindBall() {
	...
	const zone =
		document.getElementById("coze-hitbox") ||
		document.getElementById("coze-ball");
	if (!zone) return;
...
function handleToolResponse(content: string) {
	try {
		...
		if (
			...
			return;
		const expiresAt =
			typeof data.expiresAt === "number" ? data.expiresAt : jwtExpiryMs(jwt);
		if (!Number.isFinite(expiresAt)) return; // 解不出过期时间 → 拒绝解锁
		const changed = auth.token !== jwt || auth.expiresAt !== expiresAt;
		auth.token = jwt;
...
async function sendCoze(prefill?: string) {
	...
	try {
		...
		const incomplete =
			!streamOk || (last?.role === "assistant" && !last.content);
...
<style>
  ...
  .bubble-quote .quote-ico :global(svg) {
    width: 12px;
    height: 12px;
    display: block;
    fill: currentColor; /* 引用图标跟随引用行文字色随主题翻转 */
  }
